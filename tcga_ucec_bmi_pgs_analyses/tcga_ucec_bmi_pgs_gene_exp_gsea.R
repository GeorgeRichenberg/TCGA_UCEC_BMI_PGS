...
fgsea_hallmark_nes <- ggbarplot(fgsea_hallmark_plot_data, 
                                x = "pathway", 
                                y = "NES", 
  ...
  scale_x_discrete(expand = c(0.01, 0.1))
fgsea_hallmark_nes
ggsave(fgsea_hallmark_nes, file = "new_fgsea_hallmark_nes.pdf")

### Assign statistics to IL6-JAK-STAT3 lead genes from the IL6-JAK-STAT3 hallmark in MSigDB: 
...
### Plotting PGS vs IL6-JAK-STAT3 key genes:
...
ggdraw() +
  draw_plot(plot_il6, x=0, y=0.5, width = 1, height = 0.5) +
  draw_plot(plot_stat3, x=0, y=0, width = 1, height = 0.5)
...
merge_gene_exp_covars_msih_msil <- pgs %>%
  inner_join(clinical8, by = "tcgaid") %>% 
  inner_join(ancestry2, by = "tcgaid") %>% 
  inner_join(gene_exp6, by = "tcgaid")

### Run multivariable regression for MSS:
merge_gene_exp_covars_result_mss <- merge_gene_exp_covars_mss %>%
  gather(measure, value, -c(tcgaid, pgs, age, ms_status, stage, PC1:PC10)) %>%
  nest(-measure) %>%
  mutate(fit = map(data, ~ glm(value ~ pgs + age + stage + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, gaussian(link = "identity"), data = .x)),
         tidied = map(fit, tidy)) %>%
  unnest(tidied)
merge_gene_exp_covars_result_mss2 <- merge_gene_exp_covars_result_mss %>%
  filter(term == "pgs")
merge_gene_exp_covars_result_mss2 <- merge_gene_exp_covars_result_mss2 %>%
  arrange(p.value)
merge_gene_exp_covars_result_mss2$fdr <- p.adjust(merge_gene_exp_covars_result_mss2$p.value)
merge_gene_exp_covars_result_mss2$measure <- str_remove(merge_gene_exp_covars_result_mss2$measure, "\\|[0-9]*")
merge_gene_exp_covars_result_mss2 <- merge_gene_exp_covars_result_mss2 %>%
  filter(measure != "?")
merge_gene_exp_covars_result_mss2 <- merge_gene_exp_covars_result_mss2 %>% 
  distinct(measure, .keep_all = TRUE)
write_csv(merge_gene_exp_covars_result_mss2, file = "merge_gene_exp_covars_result_mss2.csv")

### View MSigDB pathways:
hallmark <- msigdbr(species = "human", category = "H")
hallmark <- split(x = hallmark$gene_symbol, f = hallmark$gs_name)
rankstats <- as_vector(abs(merge_gene_exp_covars_result_mss2$statistic))
names(rankstats) <- merge_gene_exp_covars_result_mss2$measure
set.seed(149)
fgsea_hallmark <- fgsea(pathways = hallmark,
                        stats    = rankstats,
                        minSize  = 15,
...
### Determining enriched gene-pathway associations:
fgsea_hallmark_plot_data <- fgsea_hallmark %>% 
  arrange(-padj)
fgsea_hallmark_plot_data$log <- -log10(fgsea_hallmark_plot_data$padj)
write_csv(fgsea_hallmark_plot_data, file = "fgsea_hallmark_plot_data_mss.csv")
x <- -log10(0.05)
fgsea_hallmark_bar <- ggbarplot(fgsea_hallmark_plot_data, 
                                x = "pathway", 
                                y = "log", 
                                ylab = "-log10(FDR) from GSEA", 
                                xlab = "Hallmark pathways", 
                                orientation = "horizontal") + font("x.text", size = 5) + geom_hline(aes(yintercept = x))
fgsea_hallmark_bar
ggsave(fgsea_hallmark_bar, file = "fgsea_hallmark_bar_mss.pdf")

### Run multivariable regression for MSI-H and MSI-L cases:
merge_gene_exp_covars_result_msih_msil <- merge_gene_exp_covars_msih_msil %>%
  gather(measure, value, -c(tcgaid, pgs, age, ms_status, stage, PC1:PC10)) %>%
  nest(-measure) %>%
  mutate(fit = map(data, ~ glm(value ~ pgs + age + stage + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10, gaussian(link = "identity"), data = .x)),
         tidied = map(fit, tidy)) %>%
  unnest(tidied)
merge_gene_exp_covars_result_msih_msil2 <- merge_gene_exp_covars_result_msih_msil %>%
  filter(term == "pgs")
merge_gene_exp_covars_result_msih_msil2 <- merge_gene_exp_covars_result_msih_msil2 %>%
  arrange(p.value)
merge_gene_exp_covars_result_msih_msil2$fdr <- p.adjust(merge_gene_exp_covars_result_msih_msil2$p.value)
merge_gene_exp_covars_result_msih_msil2$measure <- str_remove(merge_gene_exp_covars_result_msih_msil2$measure, "\\|[0-9]*")
merge_gene_exp_covars_result_msih_msil2 <- merge_gene_exp_covars_result_msih_msil2 %>%
  filter(measure != "?")
merge_gene_exp_covars_result_msih_msil2 <- merge_gene_exp_covars_result_msih_msil2 %>% 
  distinct(measure, .keep_all = TRUE)
write_csv(merge_gene_exp_covars_result_msih_msil2, file = "merge_gene_exp_covars_result_msi2.csv")

### View MSigDB pathways:
hallmark <- msigdbr(species = "human", category = "H")
hallmark <- split(x = hallmark$gene_symbol, f = hallmark$gs_name)
rankstats <- as_vector(abs(merge_gene_exp_covars_result_msih_msil2$statistic))
names(rankstats) <- merge_gene_exp_covars_result_msih_msil2$measure
set.seed(149)
fgsea_hallmark <- fgsea(pathways = hallmark,
                        stats    = rankstats,
                        minSize  = 15,
...
fgsea_hallmark_plot_data <- fgsea_hallmark %>% 
  arrange(-padj)
fgsea_hallmark_plot_data$log <- -log10(fgsea_hallmark_plot_data$padj)
write_csv(fgsea_hallmark_plot_data, file = "fgsea_hallmark_plot_data_msi.csv")
fgsea_hallmark_bar <- ggbarplot(fgsea_hallmark_plot_data, 
                                x = "pathway", 
                                y = "log", 
                                ylab = "-log10(FDR) association with BMI PGS", 
                                xlab = "Hallmark pathways",
                                orientation = "horizontal") + font("x.text", size = 12) + geom_hline(aes(yintercept = x))
fgsea_hallmark_bar
ggsave(fgsea_hallmark_bar, file = "fgsea_hallmark_bar.pdf")
fgsea_hallmark_nes <- ggbarplot(fgsea_hallmark_plot_data, 
                                x = "pathway", 
                                y = "NES", 
                                ylab = "Normalized enrichment score", 
                                xlab = "Hallmark pathways",
                                orientation = "horizontal") + font("x.text", size = 12)
fgsea_hallmark_nes
ggsave(fgsea_hallmark_nes, file = "fgsea_hallmark_nes.pdf")