...
#### Selecting mutational signatures with a zero count < 90%:
...
merge_mutsig_covars_msih_msil <- pgs %>%
  inner_join(clinical8, by = "tcgaid") %>% 
  inner_join(ancestry2, by = "tcgaid") %>% 
  inner_join(mutsig4, by = "tcgaid")

### Run multivariable regression for MSS cases:
merge_mutsig_covars_result_mss <- merge_mutsig_covars_mss %>%
  gather(measure, value, -c(tcgaid, pgs, age, ms_status, stage, PC1:PC10, Accuracy)) %>%
  nest(-measure) %>%
  mutate(fit = map(data, ~ glm(value ~ pgs + age + stage + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + Accuracy, quasipoisson(link = "log"), data = .x)),
         tidied = map(fit, tidy)) %>%
  unnest(tidied)
merge_mutsig_covars_result_mss2 <- merge_mutsig_covars_result_mss %>%
  filter(term == "pgs")
merge_mutsig_covars_result_mss2 <- merge_mutsig_covars_result_mss2 %>%
  arrange(p.value)
write_csv(merge_mutsig_covars_result_mss2, file = "merge_mutsig_covars_result_mss2.csv")

### Run multivariable regression for MSI-H and MSI-L cases:
merge_mutsig_covars_result_msih_msil <- merge_mutsig_covars_msih_msil %>%
  gather(measure, value, -c(tcgaid, pgs, age, ms_status, stage, PC1:PC10, Accuracy)) %>%
  nest(-measure) %>%
  mutate(fit = map(data, ~ glm(value ~ pgs + age + stage + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + Accuracy, quasipoisson(link = "log"), data = .x)),
         tidied = map(fit, tidy)) %>%
  unnest(tidied)
merge_mutsig_covars_result_msih_msil2 <- merge_mutsig_covars_result_msih_msil %>%
  filter(term == "pgs")
merge_mutsig_covars_result_msih_msil2 <- merge_mutsig_covars_result_msih_msil2 %>%
  arrange(p.value)
write_csv(merge_mutsig_covars_result_msih_msil2, file = "merge_mutsig_covars_result_msih_msil2.csv")