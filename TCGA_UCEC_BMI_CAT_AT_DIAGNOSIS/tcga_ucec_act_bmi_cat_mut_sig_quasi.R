...
#### Selecting mutational signatures with a zero count < 70%:
...
merge_mutsig_covars_result1 <- merge_mutsig_covars %>%
  gather(measure, value, -c(tcgaid, bmi_cat, age, ms_status, stage, PC1:PC10, Accuracy)) %>%
  nest(-measure) %>%
  mutate(fit = map(data, ~ glm(value ~ bmi_cat + age + ms_status + stage + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + Accuracy, quasipoisson(link = "log"), data = .x)),
         tidied = map(fit, tidy)) %>%
  unnest(tidied)
...
write_csv(merge_mutsig_covars_result2, file = "act_bmi_pgs_four_cat_mutsig_covars_result.csv")